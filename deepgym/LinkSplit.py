# Actually this is the original code of RandomLinkSplit in PyG.
# Yet to implement split according to timestamp, we must make some incisive changes.
# And this enables us to set random seed.
import warnings
...
class LinkSplit(BaseTransform):
    ...
    def __init__(
        ...
        self.order=order # My modification
    ...
    def __call__(
        ...
        for item in zip(stores, train_stores, val_stores, test_stores,
            ...
            ### My modification is wrapped in the section of 3# and will end with a #
            # Plus 2 lines in argument passing
            if is_undirected:
                ...
                if not self.order: #
                    ...
                    perm = perm[torch.tensor([i for i in range(0,perm.size(0))], device=perm.device)] #
            else:
                device = edge_index.device
                if not self.order: #
                    perm = torch.randperm(edge_index.size(1), device=device)
                else:
                    perm = torch.tensor([i for i in range(0, edge_index.size(1))], device=device)
            ###
